...
export const adminCouponsApi = {
  list: ({ page, limit, filters }) =>
    request(
      `/api/admin/coupons?${buildCouponsQuery({ page, limit, filters })}`,
    ),
  ...
  remove: (id) => request(`/api/admin/coupons/${id}`, { method: "DELETE" }),
};
...
export const adminOrdersApi = {
  list: ({ page, limit, filters }) => {
    ...
    if (filters?.archived) params.set("archived", "true"); // ADD
  ...
  bulkUpdateStatus: (ids, status) =>
    request(`/api/admin/orders/bulk-status`, {
      method: "PATCH",
      body: JSON.stringify({ orderIds: ids, orderStatus: status }),
    }),
