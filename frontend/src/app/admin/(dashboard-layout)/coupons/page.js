...
export default function CouponsPage() {
  ...
  const handleToggleStatus = async (coupon) => {
    ...
    try {
      ...
      setCoupons((prev) =>
        prev.map((c) => (c._id === coupon._id ? res.data : c)),
      );
    } catch (err) {
    ...
    <div>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button
          ...
          className="inline-flex items-center justify-center rounded-lg bg-[#CAEF96] px-4 py-2 text-sm font-medium text-black shadow-sm hover:bg-[#CAEF96]-80"
        >
          + Create Coupon
        </button>
      </div>
