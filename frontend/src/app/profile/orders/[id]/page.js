...
export default function OrderDetailPage() {
    ...
    if (loading) {
        return <div className="max-w-4xl mx-auto px-4 py-16 text-center text-gray-500">Loading order...</div>;
    }
    if (!order) {
        return <div className="max-w-4xl mx-auto px-4 py-16 text-center text-gray-500">Order not found.</div>;
    }

    const isCancelled = order.orderStatus === 'cancelled';
    const currentStepIndex = STEPS.indexOf(order.orderStatus);
    const canCancel = !isCancelled && !['shipped', 'delivered'].includes(order.orderStatus);

    return (
        <div className="max-w-4xl mx-auto px-4 py-10">
            <button onClick={() => router.push('/profile/orders')} className="text-sm text-gray-500 mb-6">
                ← Back to Orders
            </button>
                ...
                <div className="flex items-center justify-between mb-10">
                    {STEPS.map((step, i) => (
                        <div key={step} className="flex-1 flex flex-col items-center relative">
                            ...
                            {i <= currentStepIndex ? (
                                <CheckCircle2 size={24} className="text-black relative z-10 bg-white" />
                            ) : (
                                <Circle size={24} className="text-gray-300 relative z-10 bg-white" />
                            )}
                            <span className="text-xs mt-2 capitalize text-gray-600">{step}</span>
                        </div>
                    ))}
                </div>
            ) : (
