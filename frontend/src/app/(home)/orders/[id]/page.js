...
const STEP_DATE_FIELD = {
  pending: "createdAt",
  confirmed: "paidAt",
  shipped: null, // no dedicated timestamp field on the model — falls back to updatedAt
...
function OrderSkeleton() {
  return (
    <div className="max-w-4xl mx-auto px-4 py-10 animate-pulse">
        ...
        <div className="flex items-start justify-between relative z-10">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="flex-1 flex flex-col items-center">
              ...
              <div className="h-2 w-12 bg-gray-200 rounded-md mt-2"></div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="border border-gray-100 rounded-2xl divide-y mb-8">
        {[1, 2].map((i) => (
          <div key={i} className="flex items-center gap-4 p-5">
            ...
            <div className="h-5 w-16 bg-gray-200 rounded-md shrink-0"></div>
          </div>
        ))}
      </div>

      {/* Price Breakdown */}
      <div className="border border-gray-100 rounded-2xl p-6 space-y-4 mb-6">
        {[1, 2, 3].map((i) => (
      ...
      {/* Shipping Address */}
      <div className="border border-gray-100 rounded-2xl p-6 flex gap-3">
        <div className="h-5 w-5 bg-gray-200 rounded-full shrink-0"></div>
        <div className="flex-1 space-y-2">
          <div className="h-5 w-32 bg-gray-300 rounded-md mb-2"></div>
          <div className="h-4 w-3/4 max-w-sm bg-gray-200 rounded-md"></div>
          ...
          <div className="flex items-start justify-between relative">
            {STEPS.map((step, i) => {
              ...
              return (
                ...
                >
                  {/* Tooltip Pop-up */}
                  <div className="absolute -top-10 opacity-0 group-hover:opacity-100 transition-all duration-200 ease-in-out bg-gray-900 text-white text-[11px] py-1.5 px-3 rounded-lg shadow-lg whitespace-nowrap z-50 pointer-events-none translate-y-1 group-hover:-translate-y-1">
                    {step.desc}
                  ...
                  {/* Icon */}
                  <div
                    className={`relative z-10 h-8 w-8 rounded-full flex items-center justify-center transition-colors duration-300 ${
                      done
                        ? "bg-black text-white"
                  ...
                  {/* Text underneath */}
                  <span
                    className={`text-xs mt-2 font-medium text-center transition-colors duration-300 ${done ? "text-black" : "text-gray-400"}`}
                  >
                    {step.label}
                  </span>
                  {done && dateVal && (
              ...
              {isDelivered &&
                (item.hasReviewed ? (
                  ...
                  <Link
                    ...
                    className="inline-block text-xs font-semibold text-black underline hover:text-gray-600 transition-colors"
                  >
