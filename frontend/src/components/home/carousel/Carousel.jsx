...
export default function Carousel({ items }) {
    ...
    return (
        <div className="w-full h-full relative transition-all duration-300 ease-in-out mt-1" onMouseEnter={stopAutoSlide} onMouseLeave={startAutoSlide}>
                ...
                {isLoading ? (
                    ...
                    <>
                        {items.map((item, index) => {
                            ...
                            return (
                                <div key={item.id} className={`absolute inset-0 transition-opacity duration-1000 ease-in-out ${isActive ? 'opacity-100 z-10' : 'opacity-0 z-0'}`} style={{
                        ...
                        <button className="absolute cursor-pointer left-10 bottom-2 transform -translate-y-1/2 bg-primary hover:bg-opacity-60 text-black px-9 py-5 rounded-full z-20 transition">
                            View Details
                        </button>
                    </>
                )}
