  ...
  if (notifications.length === 0 && !loading) {
    return (
      <div className="flex flex-col items-center justify-center text-center py-24 border border-dashed border-[#E5E5E5] rounded-2xl">
        ...
        <h3 className="text-lg font-bold uppercase tracking-tight text-[#111] mb-1">
          All quiet
        </h3>
        <p className="text-sm text-[#6F6F6F] max-w-xs">
          New products, discounts, coupons, and review activity will show up
          here.
        </p>
      </div>
    );
      ...
      <ul className="divide-y divide-[#E5E5E5]">
        {notifications.map((n) => {
          ...
          return (
            <li
              key={n._id}
              className={`flex gap-4 py-4 ${!n.isRead ? "bg-[#FF5A1F]/5 -mx-4 px-4 rounded-lg" : ""}`}
            >
