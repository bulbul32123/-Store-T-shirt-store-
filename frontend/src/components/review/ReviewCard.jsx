...
function ReviewMenu({ review, onEdit, onReportSelect }) {
    ...
    return (
        <div ref={ref} className="relative">
            ...
            {open && (
                <div className="absolute right-0 top-8 z-20 bg-white border border-gray-100 rounded-xl shadow-lg py-1 w-44 text-sm">
                    {review.isOwner && (
                        <button
                    ...
                    {!review.isOwner && (
                        <>
                            ...
                            {REPORT_REASONS.map(reason => (
                                <button
                                    key={reason}
                                    onClick={() => { setOpen(false); onReportSelect(reason); }}
                                    className="w-full flex items-center gap-2.5 px-3 py-2 text-gray-600 hover:bg-rose-50 hover:text-rose-700 transition-colors"
                                >
...
export default function ReviewCard({ review }) {
    const { handleLike, handleDislike, handleReport, openForm, openMediaModal } = useReviews();
    ...
    const mediaItems = [
        ...(review.images || []).map(img => ({ type: 'image', url: img.url || img })),
    ...
    return (
        <div className="bg-white rounded-2xl border border-gray-100 p-5 hover:border-gray-200 transition-colors">
            ...
            <div className="flex items-start justify-between gap-3 mb-3">
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="h-10 w-10 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex-shrink-0 flex items-center justify-center overflow-hidden">
                        {review.userAvatar ? (
                            <img src={review.userAvatar} alt="" className="h-10 w-10 object-cover" />
                        ) : (
                    ...
                    <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                            <span className="font-semibold text-gray-900 text-sm">{review.displayName}</span>
            ...
            {/* ── Tags (size / color / fit) ────────────────────────────── */}
            {(review.sizePurchased || review.colorPurchased || review.fitFeedback) && (
                <div className="flex flex-wrap gap-1.5 mb-3">
                    ...
                    {review.fitFeedback && (
                        <span className={`text-xs px-2 py-0.5 rounded-full font-semibold ${FIT_COLORS[review.fitFeedback] || 'bg-gray-100 text-gray-600'}`}>
                            {review.fitFeedback}
                        </span>
                    )}
                </div>
